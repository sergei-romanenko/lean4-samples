...
proof.
  ...
  have (n = div2 (double n)) by double_div2.
         ~= (div2 (double m)) by H.
...
proof.
  ...
  have sqrt2:((sqrt (INR 2%nat) * sqrt (INR 2%nat))= INR 2%nat:>R)
         by sqrt_def.
  have (INR (Zabs_nat p * Zabs_nat p) 
     = (INR (Zabs_nat p) * INR (Zabs_nat p))) 
    by mult_INR.
    ~=  (IZR p* IZR p) by square_abs_square.
    ~=  ((IZR p/INR q*INR q)*(IZR p/INR q*INR q)) by triv. 
        (* we have to factor because field is too weak *)
